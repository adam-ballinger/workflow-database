/**
 * @file Utility functions for working with structured data.
 *
 * Provides functions to read and write CSV, JSON, and HTML files,
 * manipulate arrays of objects through filtering, updating, and pivoting,
 * generate random hex strings, and perform simple data aggregation.
 *
 * @module data-util
 */
const fs = require("fs");
...
function pivot(data, rowKey, colKey, valueKey) {
  ...
  data.forEach((item) => {
    const rowVal = item[rowKey]; // e.g., Dept (422)
    const colVal = item[colKey]; // e.g., Date ("2025-03")
    const value = item[valueKey]; // e.g., Labor Hours (342)

    // Ensure the row exists in the map
    if (!map.has(rowVal)) {
      map.set(rowVal, { [rowKey]: rowVal }); // Initialize row with Dept
    }
